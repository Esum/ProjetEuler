...
mains = [ (l.split(' ')[:5], l.split(' ')[5:]) for l in """8C TS KC 9H 4S 7D 2S 5D 3S AC
...
def indices_carte(carte):
    if carte[0] in [str(k) for k in range(2, 10)]:
        col = int(carte[0])
    if carte[0] == 'T':
        col = 10
    if carte[0] == 'A':
    ...
    if carte[0] == 'K':
        col = 13
    row = ['C', 'S', 'H', 'D'].index(carte[1])
    return row, col


def highest(main):
    m = [indices_carte(carte)[1] for carte in main]
    m.sort()
    return m

def has_flush(main):
    if all(carte[1] == main[0][1] for carte in main):
        indices = [indices_carte(carte)[0] for carte in main]
        indices.sort()
        return True, indices
    else:
        return False, None
...
def has_four(main):
    if len(main) < 4:
        ...
        for c in combinations(main, 4):
            if all(carte[0] == c[0][0] for carte in c):
                return True, indices_carte(c[0])[1]
        return False, None
...
def has_pair(main):
    if len(main) < 2:
        ...
        for c in combinations(main, 2):
            if all(carte[0] == c[0][0] for carte in c):
                return True, indices_carte(c[0])[1]
        return False, None

def has_2pairs(main):
    has, num = has_pair(main)
    if has:
        m = list(main)
        for carte in main:
            if indices_carte(carte)[1] == num:
                m.remove(carte)
        has, num2 = has_pair(m)
        if has:
            return True, (min(num, num2), max(num, num2))
    return False, (None, None)

def has_straight(main):
    if len(main) < 5:
        return False
    else:
        m = [indices_carte(carte)[1] for carte in main]
        m.sort()
        if m[-1] == 13 and m[-2] != 12:
            m = [1] + m[:-1]
        for k in range(4):
            if m[k+1] - m[k] != 1:
                return False, None
...
def has_full(main):
    ...
    if not brelan:
        ...
        for carte in main:
            if indices_carte(carte)[1] == num:
                m.remove(carte)
        pair, num2 = has_pair(m)
        if pair:
            return True, (num2, num)
        return False, (None, None)

def has_straightflush(main):
    if has_flush(main)[0]:
        return has_straight(main)
    else:
        return False, None


def best(main):
    has, num = has_straightflush(main)
    if has:
        return 10, highest(main)
    has, num = has_four(main)
    if has:
        return 9, num
    has, (num1, num2) = has_full(main)
    if has:
        return 8, (num1, num2)[-1::-1]
    has, indices = has_flush(main)
    if has:
        return 7, indices[-1::-1]
    has, num = has_straight(main)
    if has:
        return 6, num
    has, num = has_brelan(main)
    if has:
        return 5, num
    has, (num1, num2) = has_2pairs(main)
    if has:
        return 4, (num1, num2)[-1::-1]
    has, num = has_pair(main)
    if has:
        return 3, num
    else:
        return 2, highest(main)[-1::-1]


def solve54(m=mains):
    win1 = 0    
    for main1, main2 in m:
        power1, b1 = best(main1)
        power2, b2 = best(main2)
        if power1 > power2:
            win1 += 1
        elif power1 == power2:
            if b1 == b2:
                print(main1, main2, sep=' | ')
            elif b1 > b2:
                win1 += 1
    return win1